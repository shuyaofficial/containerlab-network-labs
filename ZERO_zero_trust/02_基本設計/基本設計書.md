...
| D-7 | NW-ZT トラックで IOL 連携するか | する（L7 トラックとは**別トラック**として解禁） | NAC/μセグは L2/L3 が主眼で IOL 必須。**D-1 は L7 認可トラックの決定であり本トラックには及ばない**。トラックを分けることで ZERO 本体の docker 完結（D-1）を維持しつつ、NW エンジニア本業のネットワーク側 ZT を扱える | L7 トラックに IOL を混ぜる（D-1 と矛盾し再現性低下）／ NW-ZT を完全別テーマに分離（設計ハブ集約の利点を失う） |

> **トラック分離の考え方**: 本テーマ ZERO は2トラック構成になった。**L7 トラック**（Phase 0-6、docker 完結、D-1）と、**NW-ZT トラック**（N1-N4、IOL 連携解禁、D-7）。両者は独立して読め、Phase 番号（L7）と N 番号（NW-ZT）は混同しない。NW-ZT トラックの設計は [NW-ZT_トラックロードマップ](NW-ZT_トラックロードマップ.md)・[NW-ZT_ギャップ分析](NW-ZT_ギャップ分析.md)・[NW-ZT_論理構成設計](NW-ZT_論理構成設計.md) に、実装は既存の番号テーマ（[ロードマップ PHASE2](../../ロードマップ/PHASE2_MODERN_ENTERPRISE.md) 参照）に委譲する。
